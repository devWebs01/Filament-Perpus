...
class TransactionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Status mapping
        $pendingStatus = $statuses->get('Menunggu Persetujuan');
        ...
        // Pending requests (Menunggu Persetujuan)
        for ($i = 0; $i < 5; $i++) {
            $user = $users->random();
            $book = $books->random();
            $requestDate = now()->subHours(rand(1, 23)); // Create requests within last 23 hours

            $transactions[] = [
                'code' => 'TRX-'.date('Ymd', $requestDate->timestamp).'-'.strtoupper(Str::random(4)),
                'user_id' => $user->id,
                'book_id' => $book->id,
    ...
    private function createDemoTransactions(): void
    {
        ...
        $demoUser = User::where('email', 'budi.santoso@testing.com')->first();
        ...
        $overdueUser = User::where('email', 'siti.aminah@testing.com')->first();
        ...
        // Demo 4: Pending request
        $pendingBook = Book::where('title', 'Database Relasional dengan MySQL')->first();
        $pendingUser = User::where('email', 'ahmad.fadli@testing.com')->first();

        if ($pendingBook && $pendingUser && isset($pendingStatus)) {
            Transaction::create([
                'code' => 'DEMO-PENDING-001',
