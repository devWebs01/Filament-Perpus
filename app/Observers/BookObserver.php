 ...
 * 2. Generate barcode image dan simpan path ke field barcode
 *
 * Format field barcode: "barcodes/book_1_BOK_44SVZVB2T36X.png"
 */
class BookObserver
{
    ...
    {
        try {
            // Gunakan barcode_code yang disimpan saat creating, atau generate baru
            $barcodeCode = $book->barcode_code ?? 'BOK_'.uniqid();
            ...
            // Update field barcode dengan path image
            $book->update([
                'barcode' => $barcodePath,
            ]);
    ...
    public function updating(Book $book): void
    {
        ...
        if (empty($book->getOriginal('barcode')) && ! $barcodeWasChanged) {
            try {
                $barcodeCode = 'BOK_'.uniqid();
    ...
    public function deleted(Book $book): void
    {
        try {
            if (! empty($book->barcode)) {
                $this->barcodeService->deleteBarcode($book->barcode);
            }
